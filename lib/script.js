...
const turnUrlsIntoElements = (response) => {
  ...
  return response.map((link) => $(`<tr class='link-row'><td><a href='${link.id}' target='_blank'>${host}/${link.id}</a></td><td>Links to: ${link.longUrl}</td><td>Clicks: ${link.clicks}</td><td>Created: ${new Date(link.timestamp)}</td></tr>`) );
};
